...
)
{
    ...
    return (DSTATUS)Loader::sdFs->disk_initialize();
}
...
)
{
    ...
    return (DSTATUS)Loader::sdFs->disk_status();
}
...
)
{
    ...
    if (Loader::sdFs->disk_read((uint8_t*)buff, sector, count))
        return RES_PARERR;
    else
        return RES_OK;
}
    ...
    switch(ctrl) {
        ...
        case GET_SECTOR_COUNT:
            if(Loader::sdFs == NULL) {
                ...
                DWORD res = Loader::sdFs->disk_sectors();
                if(res > 0) {
